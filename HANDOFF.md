...
- **Framing = HEAD-AND-SHOULDERS, like Braulo** (head in the top ~40–55%, shoulders/torso filling the rest). Do NOT crop tight to the head/collar — that reads as "zoomed in" (the v1 RBG/Marty mistake).
- **If the ref is a tight bust that fills its own frame edge-to-edge** (e.g. RBG's "Face Clean and Shoulders"), a full crop STILL reads zoomed-in because there's no margin around the subject. Fix = **pad the source with the sampled bg color** before converting so the subject sits smaller: sample bg = median of top/left/right edges; add ~120px top/bottom and side-pad to reach 1.2 aspect; paste the ref onto that cream canvas; then run `ref_to_bust` on the padded file with the full canvas as `--crop` (use explicit `W,H`, not a huge sentinel — PIL has a decompression-bomb guard). This is how the final RBG (`4cde782`) was framed. Ideal long-term: Nicolas frames refs with a bit of headroom/shoulder margin so no padding is needed.
