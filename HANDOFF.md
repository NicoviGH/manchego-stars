...
1. **Menu mouth-hole (root-caused, fix is tractable).** Pressing Start / opening the status-menu face shows a missing rectangle where the mouth is. Cause: the menu uses `PutFace80x72_Standard` ([fireemblem8u/src/face.c:660](fireemblem8u/src/face.c)), which fills the face from a TSA pattern then **stamps the mouth from tile indices `0x1C-0x1F` (sheet row 0, cols 28-31) and `0x3C-0x3F` (row 1)** — a SECOND copy of the closed mouth that vanilla sheets carry but our `portrait_tool.encode` never writes (we only pack the 96×96 conversation OAM tiles 0x00/0x08/0x10/0x50/0x14/0x16). Verified vanilla Lute HAS the mouth at 0x1C-0x3F. **Fix:** extend `encode` (or static `generate`) to also write the neutral mouth into sheet px region (224,0)-(256,16). Needs the exact mouth crop + an in-game check with Nicolas (offline 80×72 OAM render does NOT exercise this path — it uses the TSA+stamp, so verify on hardware/mGBA, not just a montage).
...
## NEXT STEPS (priority)
1. **Menu mouth-hole fix** (#1 above) — small, high-value, root cause known.
2. **Chibi pass** (#2).
