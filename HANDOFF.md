...
- **Use `tools/autoframe.py` for every ref** (don't hand-crop): it detects the subject, composites it onto a flat-bg 1.2-aspect canvas — bottom-anchored, ~16% headroom, ~12% side margins — then prints the exact `ref_to_bust.py … --crop 0,0,W,H` command to run on the framed file. This fixes the "zoomed-in / cut-off / floating" issues hit on RBG+Marty v1–v3 in one step. Sanity-check after: opaque rows should reach ~row 78 with ~11-16 rows of top headroom.
