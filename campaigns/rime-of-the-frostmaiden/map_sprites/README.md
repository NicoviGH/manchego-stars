...
## Prologue guest (Hlin)
- `hlin-trollbane.png` (idle 16x48) / `hlin-trollbane_mu.png` (walk 32x480): **Fighter (F) Axe** by **Alusq**, from Klokinator FE-Repo (`Map Sprites/Infantry - (Axe) Fighters and Warriors`). RAW FE8 format with Alusq palette — needs cast-palette conversion + guest-slot wiring (inject_map_sprites only handles the PORTRAIT_MAP cast today). Inert until then.
