...
Authored FE8 talking-portrait busts, one `<unit>.png` per cast/recruit/guest.
...
- **Guest portraits (campaign NPCs):** the custom-art-everywhere rule covers the CAST; chapter
  guests may instead vendor an FE-Repo mug (originals + credits in `vendor/`, regenerated by
