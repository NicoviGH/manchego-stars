...
### The gate is the spine plus the last two chapters; depth lives in the chapter suite (2026-08-23, #302)
...
### A blanket `pkill` is a serial-world habit, and parallel dispatch turned it into a saboteur (2026-08-23, #310)
...
### The asset table is addressed by a u8, so ch06 RECLAIMS a slot rather than appending (2026-09-03, #26)
...
### A hosted chapter inherits its host slot's FOG, and nothing guards that (2026-09-03, #26)
...
principle applied where it has always pointed — FE8 is the spine, and where vanilla has an
answer we take it rather than inventing a smaller game.

This closes a question that had never been ASKED, which is why it is worth an entry rather than
a checkbox: the difficulty select screen has been live since ch01 (New Game passes through
`ProcScr_NewGameDifficultySelect`), so a player could always pick Difficult. What they got was
never chosen by us.
...
### A chapter's status is DERIVED, and HANDOFF stops carrying it (2026-08-23, #312)
...
### The tileset's one home had three more callers, and they were the ones writing TILES (2026-09-16, #374)
...
### Three routes to one sidecar, and the fix is that they must AGREE (2026-09-16, #373)
...
**The build's route is DERIVED from `build_campaign`'s source**, the way `_injection_call_sequence`
already reads it — a hand-kept table in `check.py` would be a fourth place to disagree about the
same fact, and a newly hosted chapter joins this gate the moment its injector registers a map.
Matched per injector body rather than across the file, so a registration cannot be attributed to
whatever `def` happened to precede it.

**Two silences are refused, and one is kept.** An injector whose name this guard cannot map to a
chapter is REPORTED, because from outside "the guard skipped it" and "there was nothing to check"
look identical (the `_tile_change_injectors_seen` lesson). A `CHxx_LAYOUT` the build does not
define is reported too. What stays a legitimate skip is a chapter whose map is painted but not yet
hosted — ch06 lived in exactly that state from #331 until #364, and a guard that reds the build
through a normal authoring window is a guard that gets bypassed. The covered set is assertable, so
a guard that quietly compares nothing cannot pass for a guard that compares everything.
