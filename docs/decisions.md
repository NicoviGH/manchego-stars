...
Also learned: **for a 1:1 retile the spatial layer is NOT gated on the ROM.** Vanilla `.xPosition`/`.yPosition`/`.ai` (Saar is `GuardTileAI`) live in the decomp and are readable ROM-free, and `difficulty.py` already threads a `terrain_avoid` parameter through every metric — nothing has ever passed it a value, so every reading silently assumes open ground. Wiring map terrain into it is outstanding work, not a blocked tier.
