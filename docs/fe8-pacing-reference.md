...
for: it is a MINIMUM over the recruits on the field, not "the newest", because once a recruit
can join above level 1 those stop being the same unit. `benched` / `typical` / `fed` are three shares of
