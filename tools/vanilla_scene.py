...
Pairs an eventscript's scene blocks (TEXTSHOW ids, in order) with the decoded
message bodies in texts/texts.txt, so our authored scenes can be staged beside
the vanilla ones for pacing reference (dialogue-pass §Inputs #4).
...
def scene_text_ids(path, name_fragment=None):
    """[(scene_name, [msg_id, ...])] in file order."""
    ...
    for m in re.finditer(r'CONST_DATA EventListScr (\w+)\[\] = \{(.*?)\n\};', src, re.S):
        ...
        if name_fragment and name_fragment.lower() not in name.lower():
            continue
        ids = [int(x, 16) for x in re.findall(r'TEXTSHOW\((0x[0-9A-Fa-f]+)\)', body)]
        if ids:
            out.append((name, ids))
    return out
