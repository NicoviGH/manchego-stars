...
REAL_SOURCE = open(os.path.join(check.REPO, 'tools', 'build_campaign.py'),
                   encoding='utf-8').read()
PREVIEW_MAPS = os.path.join(check.REPO, 'campaigns', 'rime-of-the-frostmaiden', 'maps')


def _rows(text=None, chapters=None):
    return check._sidecar_routes(text if text is not None else REAL_SOURCE,
                                 chapters if chapters is not None else list(check._chapters()),
                                 PREVIEW_MAPS)


def _violations(text=None, chapters=None):
    return check._sidecar_route_violations(_rows(text, chapters))
...
class TheThreeRoutesAgree(unittest.TestCase):
    ...
    def test_every_hosted_chapter_is_actually_compared(self):
        """A guard that compares nothing passes too, so the covered set is asserted."""
        compared = [short for short, _, routes, _ in _rows() if len(routes) == 3]
    ...
    def test_a_build_registering_another_chapters_layout_is_caught(self):
        ...
        doctored = REAL_SOURCE.replace(
            '_register_chapter_map(maps_dir, CH02_LAYOUT', '_register_chapter_map(maps_dir, CH04_LAYOUT')
        self.assertNotEqual(doctored, REAL_SOURCE)
    ...
    def test_a_yaml_map_file_that_drifts_from_the_registered_layout_is_caught(self):
        ...
        for rel, d in check._chapters():
            if str(d.get('id', '')).startswith('ch05'):
                d = dict(d, map=dict(d['map'], file='maps/ch05-renamed-by-hand.mar'))
            chapters.append((rel, d))
    ...
    def test_a_nested_map_file_the_preview_flattens_is_caught(self):
        ...
        chapters = []
        for rel, d in check._chapters():
            if str(d.get('id', '')).startswith('ch03'):
                d = dict(d, map=dict(d['map'], file='maps/caves/ch03-the-termalaine-mine.mar'))
            chapters.append((rel, d))
        bad = _violations(chapters=chapters)
        self.assertTrue(bad)
    ...
    def test_a_painted_but_unhosted_chapter_is_skipped_with_a_reason(self):
        ...
        doctored = REAL_SOURCE.replace(
            '_register_chapter_map(maps_dir, CH06_LAYOUT', '_register_nothing(maps_dir, CH06_LAYOUT')
        self.assertNotEqual(doctored, REAL_SOURCE)
        self.assertEqual(_violations(text=doctored), [])
        note = [n for short, _, _, n in _rows(text=doctored) if short == 'ch06'][0]
        self.assertIn('no injector', note)
    ...
    def test_an_injector_the_guard_cannot_attribute_is_reported_not_skipped(self):
        ...
        doctored = REAL_SOURCE.replace(
            'def inject_ch04(', 'def inject_ch04_stage_two(', 1)
        bad = _violations(text=doctored)
        self.assertTrue(bad)
        self.assertIn('inject_ch04_stage_two', bad[0])

    def test_a_layout_constant_the_build_does_not_define_is_reported(self):
        doctored = REAL_SOURCE.replace(
            '_register_chapter_map(maps_dir, CH01_LAYOUT', '_register_chapter_map(maps_dir, CHZZ_LAYOUT')
        bad = _violations(text=doctored)
        self.assertTrue(bad)
        self.assertIn('CHZZ_LAYOUT', bad[0])
