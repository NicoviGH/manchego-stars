...
def _patch_banim_spell_palette_tint():
    ...
    if 'struct BanimSpellPaletteTint' not in header:
        ...
        decl = ('enum BanimSpellPaletteTintId {\n'
                ...
                '    BANIM_SPELL_TINT_CYAN = 3,\n'
                '    BANIM_SPELL_TINT_GOLD = 4,\n'
                '};\n\n'
    ...
    if 'BanimSpellPaletteCopy' not in utils:
        ...
        support = ('static u16 BanimSpellTintGreen(u16 color)\n'
                   ...
                   '        | (high << 10);\n'
                   '}\n\n'
                   # Flame cyan: red suppressed, GREEN and BLUE both pinned to the highlight so
                   # the tint reads as a bright equal-green-blue cyan (RGB 31,219,219), not the
                   # blue-dominant navy-ice of BanimSpellTintBlue.
                   'static u16 BanimSpellTintCyan(u16 color)\n'
                   '{\n'
                   '    int r = color & 0x1F;\n'
                   '    int g = (color >> 5) & 0x1F;\n'
                   '    int b = (color >> 10) & 0x1F;\n'
                   ...
                   '        | (high << 10);\n'
                   '}\n\n'
                   # Gold: the MIRROR of the cyan above -- red pinned to the highlight and
                   # green just under it (a warm gold, not a lemon yellow), with blue
                   # suppressed the way cyan suppresses red. Exists so two casters sharing
                   # one donor AnimConf can still be told apart by the colour they cast;
                   # which characters those are is campaign data, not engine business.
                   'static u16 BanimSpellTintGold(u16 color)\n'
                   '{\n'
                   '    int r = color & 0x1F;\n'
                   '    int g = (color >> 5) & 0x1F;\n'
                   '    int b = (color >> 10) & 0x1F;\n'
                   ...
                   '        else if (gMSSpellTint == BANIM_SPELL_TINT_CYAN)\n'
                   '            dst[i] = BanimSpellTintCyan(src[i]);\n'
                   '        else if (gMSSpellTint == BANIM_SPELL_TINT_GOLD)\n'
                   '            dst[i] = BanimSpellTintGold(src[i]);\n'
                   '        else\n'
