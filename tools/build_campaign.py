...
PATCHED_DECOMP_FILES = ['texts/texts.txt', 'src/data_characters.c', 'src/portrait_data.c',
                        ...
                        # regenerated by inject_opening_montage on MONTAGE=1 builds
                        'src/opsubtitle.c'] + [
...
def _cut_boot_intro(montage=False):
    ...
    inject_opening_montage replaces its seven card slides with our lore crawl. Cut (a)
    stays in all builds (the attract reel is vanilla promo content), and (c) stays
    until the slice's Icewind Dale world-tour replaces it."""
...
def inject_opening_montage(campaign, verbose=True):
    ...
    if verbose:
        print('  lore crawl: %d slides re-rendered from %s; LUT retimed'
              % (len(cards), os.path.relpath(montage_yaml, REPO)))
