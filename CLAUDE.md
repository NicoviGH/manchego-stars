...
| `make matrix [SUITE=…]` | The live regression matrix (`tools/playtest/matrix.py`): builds each ROM configuration at most once, runs its scenarios, prints a verdict table. `SUITE=gate` (default), a chapter suite, or `all`. A green scenario whose inputs are byte-identical is served from the **verdict cache** and does not re-run (`source: cached`; `--no-verdict-cache`/`MX_NO_CACHE=1` opts out) — `--dry-run` says what would actually run, for free. Invalidation is **build-attributed**: the build records what each injection step wrote, so a ch05 edit re-runs ch05's scenarios and caches the rest (measured: 6 of 20). **Run the smallest set that covers what you changed** — chapter suite, or `matrix.py run --scenarios a,b,c`. **Do NOT run the full gate locally** (Nicolas, 2026-08-10; #255 phase 3 is what retires this rule) and never run anything after a merge. **On a failure, spend one INSTRUMENTED run, not one run per guess.** All of it: `decisions.md` → "Playtest runs are the most expensive thing in this repo". |
