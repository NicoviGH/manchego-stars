...
| `make matrix [SUITE=…]` | The live regression matrix (`tools/playtest/matrix.py`): builds each ROM configuration at most once, runs its scenarios, prints a verdict table. `SUITE=gate` (default), a chapter suite, or `all`. A green scenario whose inputs are byte-identical is served from the **verdict cache** and does not re-run (`source: cached`; `--no-verdict-cache`/`MX_NO_CACHE=1` opts out) — `--dry-run` says what would actually run, for free. **Run the smallest set that covers what you changed** — chapter suite, or `matrix.py run --scenarios a,b,c`. **Do NOT run the full gate locally** (Nicolas, 2026-08-10; the cache still keys on the ROM, so a `build_campaign.py` edit invalidates every row — #255 phase 2 is what lifts this) and never run anything after a merge. **On a failure, spend one INSTRUMENTED run, not one run per guess.** All of it: `decisions.md` → "Playtest runs are the most expensive thing in this repo". |
