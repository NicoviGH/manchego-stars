...
| `make matrix [SUITE=…]` | The live regression matrix (`tools/playtest/matrix.py`): builds each ROM configuration at most once, runs its scenarios, prints a verdict table. `SUITE=gate` (default), a chapter suite, or `all`. **Run the smallest set that covers what you changed** — chapter suite (~11s cached), or `matrix.py run --scenarios a,b,c`. **Do NOT run the full gate locally** (Nicolas, 2026-08-10) and never run anything after a merge. **On a failure, spend one INSTRUMENTED run, not one run per guess.** Both rules and what they cost us: `decisions.md` → "Playtest runs are the most expensive thing in this repo"; the cheap-gate fix is #255. |
